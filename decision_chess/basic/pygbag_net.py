...
class aio_sock:
    ...
    def send(self, *argv, **kw):
        self.socket.send(*argv, **kw)

    def recv(self, *argv):
        return self.socket.recv(*argv)
...
class Node:
    ...
    def __init__(self, gid=0, groupname="", host="", offline=False):
        ...
        # last joined channel by any node
        self.joined = ""
    ...
    def publish(self, spectator_publish):
        self.lobby_cmd(
            self.gid,
            self.pid,
            self.OFFER if not spectator_publish else self.SPEC_OFFER,
            self.groupname,
